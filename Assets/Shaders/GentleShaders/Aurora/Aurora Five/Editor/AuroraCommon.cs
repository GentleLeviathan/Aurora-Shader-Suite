...
{
    public enum SL_ZTest
    {
        Disabled, Never, Less, Equal, LEqual, Greater, NotEqual, GEqual, Always
    }

    public enum SL_ZWrite
    {
        Off, On
    }
    ...
    public enum Aurora_ViewSpecular_Setting
    {
        Disabled, Enabled
    }
    ...
    public enum Aurora_Light_Model
    {
        BRDF4 = 4, BRDF5 = 5, ACEL = 21
    }

    public static class AuroraCommon
    {
        public static string currentVersion = "AR5.0";
        ...
        /// <summary>
        /// Compares Aurora Version numbers.
        /// </summary>
        /// <param name="currentVersion"></param>
        /// <param name="checkVersion"></param>
        /// <returns>-1 if currentVersion is newer, 0 if both versions are identical, 1 if checkVersion is newer</returns>
        public static int CompareAuroraVersion(string currentVersion, string checkVersion)
        {
            int currentVersionNum = int.Parse(new System.String(currentVersion.Where(System.Char.IsDigit).ToArray()));
            int checkVersionNum = int.Parse(new System.String(checkVersion.Where(System.Char.IsDigit).ToArray()));

            if (currentVersionNum > checkVersionNum)
            {
                return -1;
            }
            else if(currentVersionNum == checkVersionNum)
            {
                int checkAlpha = string.Compare(currentVersion, checkVersion, System.StringComparison.OrdinalIgnoreCase);
                if (checkAlpha > 0)
                {
                    return -1;
                }
